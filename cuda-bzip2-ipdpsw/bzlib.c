...
void transfer_state_information (bz_stream *strm ) { 
...
int allocate_new_block_in_stream ( bz_stream *strm )
{
   EState *s;
...
Bool compress_all_blocks_in_stream (bz_stream* strm)
{
	UInt32 i;
	Int32 count;
	EState* s;
	/* for(i=0; i <= strm->state_fill_count; i++) { 
		...
		BZ_INITIALISE_CRC ( s->blockCRC );
		BZ2_compressBlock(s, (Bool) i == strm->state_fill_count);
                #ifdef PRINT_DEBUG
	...
	//BZ2_compressBlocks_pthreads( strm );  
	BZ2_compressBlocks( strm );  
...
int BZ_API(BZ2_bzCompress) ( bz_stream *strm, int action )
{
   ...
   s = (EState *) strm->state[strm->state_fill_count];
   // current EState is retrieved from state array of bz_stream structure
