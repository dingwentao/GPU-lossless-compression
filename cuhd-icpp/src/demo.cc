...
int main(int argc, char** argv) {
    ...
    TIMER_START(timings, "reading data from file")
        std::ifstream input(argv[2], std::ios::binary);
        //std::vector<SYMBOL_TYPE> buffer(std::istreambuf_iterator<char>(input), {});
        char tmp;
    ...
    */
    ...
    // compress the data using length-limited Huffman coding
    ...
    TIMER_START(timings, "writing compressed data to file")
        std::ofstream output(argv[3], std::ios::out | std::ios::binary);
        output.write(reinterpret_cast<char*> (compressed.get()),enc_table->compressed_size*sizeof(UNIT_TYPE));
        output.close();
        std::cout << "compressed file size = " << enc_table->compressed_size * sizeof(UNIT_TYPE) << "\n";
    TIMER_STOP
    ...
    //printf ("%d %d %d\n", *(buffer.data()), *(buffer.data()+4), *(buffer.data()+size));
    //printf ("%d %d %d\n", *(out_buf->get_decompressed_data().get()), *(out_buf->get_decompressed_data().get()+4), *(out_buf->get_decompressed_data().get()+size));
    return 0;
}
